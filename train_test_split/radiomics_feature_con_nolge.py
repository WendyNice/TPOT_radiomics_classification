...
path = r'D:\Data analysis\Tpot plugin\9_4\con_vs_lge-'
name_train = np.load(os.path.join(path, 'control_no_LGE_name_train.npy'))
name_test = np.load(os.path.join(path, 'control_no_LGE_name_test.npy'))
name_val = np.load(os.path.join(path, 'control_no_LGE_name_val.npy'))
print('control_HCM_train', len(list(name_train)), list(name_train))
print()
print('control_HCM_test', len(list(name_test)), list(name_test))
print()
print('control_HCM_val', len(list(name_val)), list(name_val))
...
for i in range(len(name)):
    ...
    if (name[i] in name_test) or (name[i] in name_val):
...
path_save = r'D:\Data analysis\Tpot plugin\9_4\con_vs_lge-\radiomics'
np.save(os.path.join(path_save, 'feature_train.npy'), train_df)
np.save(os.path.join(path_save, 'feature_test.npy'), np.array(test_df))
np.save(os.path.join(path_save, 'label_train.npy'), label_train)
np.save(os.path.join(path_save, 'label_test.npy'), np.array(label_test))
